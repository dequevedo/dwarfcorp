...
- [ ] 33. Tile wrap nas merged quads do greedy meshing (shader fix).
  Fase B.2 live (`62ed69480`/`4c89eb435`/`67b6cb2e6`) emite merged quads com UVs 0..1 esticadas ao longo do retângulo merged. Visual: cada tile estica pra cobrir uma área de `w × h` vóxeis em vez de repetir. Acontece porque `TexturedShaders.fx :: ClampTexture` faz `clamp(uv, bounds.xy, bounds.zw)` — se mandarmos UVs 0..w / 0..h, elas são grampeadas no retângulo do tile e o sampler estica. **Fix**: emitir merged quad com UVs `(0..w, 0..h)` no CPU + trocar `ClampTexture` por um wrap-within-tile estilo `bounds.xy + fmod(uv - bounds.xy, bounds.zw - bounds.xy)`. UVs já dentro do tile (caminhos per-voxel) ficam no-op (fmod de valor <w é identidade). Bounds-fudge (±0.001) evita bleed entre tiles adjacentes no atlas. Alternativa mais localizada: usar o `WrappedTextureSampler` que já existe só nos passes que renderizam merged quads. Disparar quando: usuário quiser o look mosaico-de-tiles em floors grandes em vez do look esticado atual.

- [ ] 32. Fullscreen init defensivo: Release build crasha com `DXGI_ERROR_NOT_CURRENTLY_AVAILABLE` se `settings.json` tem `"Fullscreen": true` e o DXGI não consegue adquirir fullscreen exclusivo (outro app em FS, multi-monitor estranho, display mudou desde o save). Debug não reproduz. Fix: `try/catch` em torno do init de fullscreen em `DwarfGame` — se falhar, fallback pra windowed + log warn. Zero linhas de config do usuário precisam mudar. Crash atual em `WinFormsGameWindow.EnterFullScreen` → `SharpDX.DXGI.SwapChain.SetFullscreenState`.

- [ ] 31. B.2 integração live do greedy meshing (top face).
  Kernel puro + testes + switch `Debugger.Switches.UseGreedyMeshing` já commitados. Falta: construir a máscara a partir de `VoxelChunk` iterando voxels visíveis, chamar `GreedyMeshSlice.GreedyMergeMask`, emitir merged quads como `ExtendedVertex` com position correta (corners do rect), UV tiled (0..W, 0..H — shader já faz `frac` em TextureBounds), Color/VertColor do voxel representativo (máscara garante equivalência). Hook em `GenerateSliceGeometry` sob o switch; per-voxel path inalterado pra voxels com grass/decal. Sessão dedicada próxima, quando dá pra testar visualmente e confirmar zero regressão em cena aberta.
  **Depois disso:** extender pras 5 outras faces em commits sucessivos (±X, ±Z, -Y).

- [ ] 30. L.4 per-family migrations — reescrever cada GameComponent-derived como componente/arquetipo Arch.
  Foundation pronta em `16b6832b6` (EcsWorld, SaveFormatVersion, ComponentSaveMigration skeleton, 6 testes). **Falta migrar 50+ classes em ~22 commits focados**, ordenados por dependência no DAG. Plano completo com ordem recomendada + dependências + archetype compositions em [`docs/l4_arch_migration_plan.md`](docs/l4_arch_migration_plan.md). Cada família é 1 commit: cria struct Arch + System + preenche `Migrate*` + tests + atualiza tripwire em `EcsFoundationTests`. ComponentManager só morre depois da última família.
  **Gatilho pra fechar:** todas 22 famílias migradas → bump `MetaData.CurrentSaveFormatVersion` pra 2 → delete `ComponentManager` + `GameComponent` tree.
