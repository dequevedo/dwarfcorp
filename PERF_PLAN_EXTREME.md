...
- 🚧 **L.4** — Foundation pronta (`16b6832b6`): `Arch 2.*` + `Arch.System 1.*` em DI, `DwarfCorp/ECS/EcsWorld.cs` wrapper singleton, placeholder `Transform` struct, `MetaData.SaveFormatVersion` field (default v1 pra saves antigos), `DwarfCorp/Saving/ComponentSaveMigration.cs` skeleton com TODO hooks por family. 6 testes novos cobrindo Arch roundtrip + save versioning. **Falta migrar 50+ classes GameComponent-derived em ~22 commits focados** — DAG de dependências + ordem recomendada em [`docs/l4_arch_migration_plan.md`](docs/l4_arch_migration_plan.md). ComponentManager só morre depois da última família + bump `CurrentSaveFormatVersion` pra 2.
