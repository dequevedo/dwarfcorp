...
- 🚧 **B.2** — **Integração live commitada.** Kernel + `FaceKey` (`57f290a23`) agora conectado via `GeometryBuilder-Greedy.cs`: mask de top faces por slice, merge via `GreedyMergeMask`, skip da top face no per-voxel pass pros cells consumidos. Eligibility conservativa (sem grass/decal/slope/transition-textures/fog). `UseGreedyMeshing` agora auto-habilita `UseNewVoxelGeoGen`. Métricas `Greedy.CellsMaskedThisFrame` / `Greedy.RectanglesEmittedThisFrame` / `Greedy.MergeRatio` aparecem no CSV. **Limitação conhecida**: shader `ClampTexture` clampa, não wrap — merged quad estica o tile em vez de repetir. Aceitável pro look low-poly pixelated; trocar pra `fmod` no shader fica como follow-up. **Falta validação visual** in-game (user toggles switch e avalia).
