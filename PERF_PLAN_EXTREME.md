...
- 🚧 **L.4** — Foundation pronta (`16b6832b6`) + famílias #1-#11 migradas (commits `eff2bedd4`, `<this>`): Transform, Physics, Health, Tintable+Sprite, AnimatedSprite/Layered, InstanceMesh, Inventory, Equipment, LightEmission, Sensores (Radius/Enemy/SpawnOnExplored), VoxelListener. Todas as 11 seguem o padrão "struct Arch + Migrate method + tests". Arch entities recebem um Transform no root-level + componentes foldados dos descendants via `LegacyIdToEntity` lookup. 17 testes no total (Foundation 6 + Transform 4 + Families 11). **Faltam famílias #12-#22** (Fire/Flammable, Follower/Bobber, MinimapIcon, DwarfThoughts/Egg, CreatureAI core, Dwarf archetype, Animal/monster AIs, Projectile/ResourceEntity, Fixture family, PipeNetworkObject, Balloon) — DAG em [`docs/l4_arch_migration_plan.md`](docs/l4_arch_migration_plan.md). ComponentManager só morre depois da última família + bump `CurrentSaveFormatVersion` pra 2.
...
- 🚧 **B.2** — **Integração live completa: Top + Bottom 2D + Sides 1D.** Kernel + `FaceKey` conectado via `GeometryBuilder-Greedy.cs`. Top/Bottom: merge 2D em (X,Z). N/S/E/W: merge 1D por slice via trick de foldar o eixo ortogonal no `LightHash` da FaceKey (células em rows/cols diferentes nunca comparam iguais, o kernel 2D degrada pra 1D limpo). Consumed-faces tracking via `byte[,]` bitmask (bit por `FaceOrientation`). Eligibility conservativa (sem grass/decal/slope/transition/fog). `UseGreedyMeshing` auto-habilita `UseNewVoxelGeoGen`. Métricas `Greedy.CellsMaskedThisFrame` / `Greedy.RectanglesEmittedThisFrame` / `Greedy.MergeRatio` cobrem as 6 faces. **Cross-slice 2D pras sides (parede alta = 1 quad inteiro)** pendente — exige rearchitect do per-slice build loop. **Limitação UV**: merged quads esticam tile — TODO 33.
- ✅ **B.3** — SIMD AVX2 solidity bitmap (`39d3dd397`). `SliceSolidityBitmap` scan 256 bytes da slice com `Vector256<byte>` + CompareEqual(zero) + MoveMask. 8 ops por slice, ThreadStatic bitmap reusado. Consulted pelas 6 greedy passes pra fast-reject cells empty antes de qualquer scalar work. Fallback scalar pra CPUs sem AVX2. 6 testes xUnit novos de paridade scalar/SIMD.
...
- 🚧 **C.3** — Escopo expandido por evidência (heap 1.5GB em idle, Gen 2 a cada 2s). Primeira leva commitada (`2de9f0019`): `FillClosestLights` (scratch list + instance comparer), Root.cs update tick (scratch list), `FindRootBodies` (scratch HashSet), `GcTracker.Gen0PerSecond()` + widget colorido no Render Inspector. Segunda leva (`1028ba68d`): Root.cs `Draw` deixou de andar a widget tree inteira pra chamar PostDraw (usa `PostdrawItems` agora); WorldRenderer.Render materializa renderables uma vez em scratch reutilizado + `EnumerateAllInto(List)` substitui o yield recursivo + signatures `ComponentRenderer`/`WaterRenderer` viraram `List<GameComponent>`; scissor `RasterizerState` cacheado. **Resultado medido**: [`baseline_v5_c3_post.csv`](docs/baselines/baseline_v5_c3_post.csv) com cena **2× mais densa** mostra **p95 −50%** (69→34ms), **p99 −52%** (683→329ms), hitches>50ms −46%. Hitches de 100-800ms ainda existem (3% dos frames); próxima leva: PhysicsSystem / ParticleManager / AISystem.
