...
- 🚧 **B.2** — **Integração live Top + Bottom.** Kernel + `FaceKey` (`57f290a23`) conectado via `GeometryBuilder-Greedy.cs`: masks 2D por slice pros planos XZ (Top face no y+1, Bottom face no y), merge via `GreedyMergeMask`, skip das faces consumidas no per-voxel pass via bitmask (`byte[,] consumedFaces`, bit por `FaceOrientation`). Eligibility conservativa (sem grass/decal/slope/transition-textures/fog). `UseGreedyMeshing` auto-habilita `UseNewVoxelGeoGen`. Métricas `Greedy.CellsMaskedThisFrame` / `Greedy.RectanglesEmittedThisFrame` / `Greedy.MergeRatio` no CSV. **Side faces (N/S/E/W) pendentes** — planos YX/YZ não couberam na arquitetura per-slice atual (merging em Y precisa coordenar entre slices); 1D-per-slice possível mas ROI muito menor que Top+Bottom 2D. Parked até medir ganho Top+Bottom. **Limitação UV**: shader `ClampTexture` estica tile em vez de tilar — TODO 33 ataca depois.
